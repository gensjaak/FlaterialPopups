...
(function (factory) {
	...
	var inflate = function (args) { return '<section class="' + elClassName + ' flaterialPopup_' + args.type + '" flaterialGroup="' + args.groupId + '"><div class="iconWrapper"><i class="' + args.icon + '"></i></div><div class="contenu"><header>' + args.title + '</header><p>' + args.body + '</p></div><div class="action-bar"></div></section>'; };
	...
	var flattern = function (shouldFlattern) {
		...
		var toFlattern = [];
		$allElts.each(function (index, element, $element = $(element)) {
			if ($allElts[index+1]) {
		...
		toFlattern.forEach(function (element, index) {
			element.$el.each(function (index, el, $el = $(el)) {
				$el.animate({
	...
	var FlaterialPopups = function () {
		...
		this.Popup.prototype.show = function() {
			...
			if (!Group.config.queue) {
				Group.$elts.each(function (index, element, $element = $(element)) {
			...
			tmp.each(function(index, element, $element = $(element)) {
				var ecart = 80;
				...
				if ($element.attr('flaterialGroup') == Group.id && Group.config.waitingForUser && Group.config.waitingForUserOpts) {
					...
					if (!($element.find('.action-bar > a').length == 2)) {
						$element.find('.action-bar').empty().append('<a href="javascript:void(0);" class="f-btn f-default" f-behaviour="negative">' + Group.config.waitingForUserOpts.negativeText + '</a><a href="javascript:void(0);" class="f-btn f-primary" f-behaviour="positive">' + Group.config.waitingForUserOpts.positiveText + '</a>');
	...
	FlaterialPopups.prototype.init = function(args) {
		...
		args.icon = defaultIcons[args.type] || "";
