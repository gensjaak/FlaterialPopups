...
(function (factory) {
	...
	var inflate = function (args) {
		return '<section class="' + ELCLASSNAME + ' flaterialPopup_' + args.type + '" flaterialGroup="' + args.groupId + '"><div class="iconWrapper">' + args.icon + '</div><div class="contenu"><header>' + args.title + '</header><p>' + args.body + '</p></div><div class="action-bar"></div><i class="md-close" hidden id="closePopup"><i/></section>';
	};
			...
			tmp.each(function(index, element) {
				...
				$element.animate({
					...
					if ($element.attr('flaterialGroup') == Group.id && Group.config.closeOnClick) {
						...
						$element.find('#closePopup').get(0).removeAttribute("hidden");
